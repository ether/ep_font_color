...
describe('export color styles to HTML', function () {
  ...
  context('when pad text has color inside strong', function () {
    ...
    it('returns HTML with strong and color, in any order', function (done) {
      api.get(getHTMLEndPointFor(padID))
          .expect((res) => {
            const strongInsideColorRegex = regexWithColor('red', '<strong>this is red and bold<\/strong>');
            const colorInsideStrongRegex = `<strong>${regexWithColor('red', 'this is red and bold')}<\/strong>`;
            ...
            const html = res.body.data.html;
            const foundColor = html.match(expectedStrongInsideColor) || html.match(expectedColorInsideStrong);
...
var regexWithColor = function (color, text) {
  ...
  const regex = `<span .*class=['|"].*color:${color}.*['|"].*>${text}<\/span>`;
