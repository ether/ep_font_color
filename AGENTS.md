...
`ep_font_color` runs inside Etherpad's test harness. From an etherpad checkout that has installed this plugin via `pnpm run plugins i --path ../ep_font_color`:
...
* Server: `eejsBlock_editbarMenuLeft`, `collectContentPre`, `collectContentPost`, `exportHtmlAdditionalTagsWithData`, `stylesForExport`, `getLineHTMLForExport`, `eejsBlock_dd_format`, `padInitToolbar`
* Client: `postAceInit`, `aceInitialized`, `aceEditEvent`, `aceEditorCSS`, `aceCreateDomLine`, `postToolbarInit`, `aceAttribsToClasses`, `collectContentPre`
